import { ArrowDown, Dot } from "lucide-react"

export const HeroSection = () => {
    return <section
        id="hero"
        className="relative min-h-screen flex flex-col items-center justify-center px-4"
    >
          <div className="grid grid-cols-1 gap-6 flex justify-start opacity-0 animate-fade-in-delay-4">
            <div className="gradient-border p-6 rocket-hover">
              <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-primary/10 text-primary">
                  <circle-small className="text-primary" />
                </div>
                <div>
                  <p className="text-glow text-primary">
                    Based in Stockholm, Sweden
                  </p>
                </div>
              </div>
            </div>
        </div>
        <div className="container max-w-4xl mx-auto text-center z-10">
            <div className="space-y-6">
                <h1 className="text-4xl md:text-6xl font-bold tracking-tight">
                    <span className="opacity-0 animate-fade-in">Hi, I'm </span>
                    <span className="text-primary opacity-0 animate-fade-in-delay-1">
                        {" "}
                        Pontus
                    </span>
                    <span className="text-gradient ml-2 opacity-0 animate-fade-in-delay-2">
                        {" "}
                        Karlsson
                    </span>
                </h1>

                <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto opacity-0 animate-fade-in-delay-3">
                    I create modern, responsive, and user-friendly web applications that leave a lasting impression.
                </p>

                <div>
                    <a href="#projects" className="cosmic-button opacity-0 animate-fade-in-delay-4 text-readable ">
                        View my work
                    </a>
                </div>
            </div>

        </div>
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col items-center animate-bounce">
            <span className="text-sm text-muted-foreground"> Scroll </span>
            <ArrowDown className="h-5 w-5 text-primary mt-2 animate-bounce" />
        </div>
    </section>

}
